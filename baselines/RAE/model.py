...
class RecurrentBlock(nn.Module):
	...
	def __init__(self, input_nc, output_nc, downsampling=False, bottleneck=False, upsampling=False, relu=True):
	...
	def reset_hidden(self, inp, dfac):
		...
		size[2] = int(size[2] / dfac)
		size[3] = int(size[3] / dfac)
...
class RecurrentAE(nn.Module):
	...
	def __init__(self, input_nc):
		...
		self.bottleneck = RecurrentBlock(input_nc=76, output_nc=76, bottleneck=True)
		...
		self.u1 = RecurrentBlock(input_nc=32, output_nc=3, upsampling=True, relu=False)
	...
	def reset_hidden(self):
		...
		self.bottleneck.reset_hidden(self.inp, dfac=16)
