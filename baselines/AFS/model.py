...
def print_model_structure(model, logger):
    blank = ' '
    logger.info('\t '+'-' * 95)
    logger.info('\t ' + '|' + ' ' * 13 + 'weight name' + ' ' * 13 + '|' + ' ' * 15 + 'weight shape' + ' ' * 15 + '|'
          + ' ' * 3 + 'number' + ' ' * 3 + '|')
    logger.info('\t ' + '-' * 95)
    num_para = 0
    ...
    logger.info('\t ' + '-' * 95)
    logger.info('\t ' + 'Total number of parameters: ' + str(num_para))
    logger.info('\t CUDA: ' + str(next(model.parameters()).is_cuda))
    logger.info('\t ' + '-' * 95 + '\n')
...
class DiscriminatorVGG128(nn.Module):
    def __init__(self, in_nc, base_nf, norm_type='batch', act_type='leakyrelu'):
        ...
        self.classifier = nn.Sequential(
            nn.Linear(512 * 4 * 4 * 4, 100), nn.LeakyReLU(0.2, True), nn.Linear(100, 1)
        )
        # train_width = 128, 512 * 4
        # train_width = 128, 512 * 4 * 4
        # train_width = 256, 512 * 4 * 4 * 4
